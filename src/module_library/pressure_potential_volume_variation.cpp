...
void pressure_potential_senescence::do_operation() const
{
    ...
    double V_stem_new = 0.0;

    double sene_swc = kSeneStem * W_stem;
    double potential_value_stem = std::max(stem_pressure_potential, wp_crit);

    if (kStem >= 0.0025) { // Case 2: plastic growth active

        dW_stem = F_root_stem - F_stem_leaf - sene_swc;
    ...
    } else if (kStem < 0.0025 && Stem >= 0.01) { // Case 3: elastic only
