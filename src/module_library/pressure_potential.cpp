...
void pressure_potential::do_operation() const
{
    ...
    double root_dPP = ((dW_root/(pw*root_volume) - (ext_root_z_initial + 2*ext_root_x)*(potential_value_root - wp_crit))
                    *((mod_root_x*mod_root_z)/(2*mod_root_z + mod_root_x))); // MPa

    // Change in root volume
    double dV_root_elastic = root_volume*((2*mod_root_z + mod_root_x)/(mod_root_z*mod_root_z))*root_dPP;
    double dV_root_plastic = (ext_root_z_initial + 2*ext_root_x)*(potential_value_root - wp_crit);
    ...
    //                     + (ext_root_z_initial + 2*ext_root_x)*(potential_value_root - wp_crit)); // m3 
    ...
    double stem_dPP = ((dW_stem/(pw*stem_volume) - (ext_stem_z_initial + 2*ext_stem_x)*(potential_value_stem - wp_crit))
                    *((mod_stem_x*mod_stem_z)/(2*mod_stem_z + mod_stem_x))); // MPa

    // Change in stem volume
    double dV_stem_elastic = stem_volume*((2*mod_stem_z + mod_stem_x)/(mod_stem_z*mod_stem_z))*stem_dPP;
    double dV_stem_plastic = (ext_stem_z_initial + 2*ext_stem_x)*(potential_value_stem - wp_crit);
    ...
    //                     + (ext_stem_z_initial + 2*ext_stem_x)*(potential_value_stem - wp_crit)); // m3
    ...
    double leaf_dPP = ((dW_leaf/(pw*leaf_volume) - (ext_leaf_z_initial + ext_leaf_x + ext_leaf_y_initial)*(potential_value_leaf - wp_crit))
                    *((mod_leaf_x*mod_leaf_z*mod_leaf_y)/(mod_leaf_x*mod_leaf_z + mod_leaf_z*mod_leaf_y + mod_leaf_x*mod_leaf_y)));

    // Change in leaf volume
    double dV_leaf_elastic = leaf_volume*((mod_leaf_x*mod_leaf_z + mod_leaf_z*mod_leaf_y + mod_leaf_x*mod_leaf_y)/(mod_leaf_x*mod_leaf_z*mod_leaf_y))*leaf_dPP;
    double dV_leaf_plastic = (ext_leaf_z_initial + ext_leaf_x + ext_leaf_y_initial)*(potential_value_leaf - wp_crit);
    ...
    //                                     *leaf_dPP + (ext_leaf_z_initial + ext_leaf_x + ext_leaf_y_initial)*(potential_value_leaf - wp_crit)); // m3
    ...
    double pods_dPP = ((dW_pods/(pw*pods_volume) - (3*ext_pods_x_initial)*(potential_value_pods - wp_crit))
                    *(mod_pods_x/3));

    // Change in pods volume
    double dV_pods_elastic = pods_volume*mod_pods_x*pods_dPP;
    double dV_pods_plastic = ext_pods_x_initial*(potential_value_pods - wp_crit);
    double dV_pods = dV_pods_elastic + dV_pods_plastic;
    // double dV_pods = (pods_volume*mod_pods_x*pods_dPP) + (ext_pods_x*(potential_value_pods - pods_pressure_potential));
    ...
    update(root_volume_plastic_op, dV_root_plastic);
    update(stem_volume_plastic_op, dV_stem_plastic);
    update(leaf_volume_plastic_op, dV_leaf_plastic);
    update(pods_volume_plastic_op, dV_pods_plastic);
}